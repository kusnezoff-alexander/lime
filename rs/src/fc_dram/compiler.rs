...
use super::{
    architecture::{
        subarrayid_to_subarray_address, Instruction, LogicOp, NeighboringSubarrayRelPosition,
        SubarrayId, SupportedNrOperands, ARCHITECTURE, NR_SUBARRAYS, ROW_ID_BITMASK,
        SUBARRAY_ID_BITMASK,
    },
    optimization::optimize,
    CompilerSettings, Program, RowAddress,
};
...
pub struct Compiler {
    ...
    compute_row_activations:
        HashMap<(SupportedNrOperands, NeighboringSubarrayRelPosition), (RowAddress, RowAddress)>,
...
impl Compiler {
    ...
    pub fn compile(&mut self, network: &impl NetworkWithBackwardEdges<Node = Aoig>) -> Program {
        ...
        while let Some((next_candidate, _)) = self.comp_state.candidates.pop() {
            let executed_instructions =
                &mut self.execute_next_instruction(&next_candidate, network);
            program.instructions.append(executed_instructions);

            // update new candidates (`next_candidate` is now available)
            let new_candidates =
                self.get_new_candidates(network, next_candidate.0, next_candidate.1);
        ...
        program.output_row_operands_placement = outputs
            .flat_map(|out| {
                let subarrays = self.signal_to_subarrayids.get(&out).unwrap();
                let mut placements = vec![];
                for subarray in subarrays {
                    let row_address = self
                        .comp_state
                        .value_states
                        .get(&(out, *subarray))
                        .expect("ERROR: one of the outputs hasn't been computed yet...");
    ...
    fn choose_compute_rows(&mut self) {
        for nr_operands in SupportedNrOperands::iter() {
            for sense_amp_position in NeighboringSubarrayRelPosition::iter() {
                let possible_row_combis = ARCHITECTURE
                    .sra_degree_to_rowaddress_combinations
                    .get(&nr_operands)
                    .expect("Given Architecture doesn't support SRA of {nr_operands} operands");
                let best_row_combi = possible_row_combis.iter().fold(
                    possible_row_combis[0],
                    |best_row_combi, next_row_combi| {
                ...
                self.compute_row_activations
                    .insert((nr_operands, sense_amp_position), best_row_combi);
            }
        }
    }
    ...
    fn place_constants(&mut self, program: &mut Program) {
        ...
        for subarray in 0..NR_SUBARRAYS {
            for constant in Self::CONSTANTS {
                let next_free_row = self
                    .comp_state
                    .free_rows_per_subarray
                    .get_mut(&SubarrayId(subarray))
                    .and_then(|v| v.pop())
                    .expect("No free rows in subarray {subarray} :(");
                self.comp_state.constant_values.insert(
                    constant,
                    next_free_row.local_rowaddress_to_subarray_id(SubarrayId(0)),
                );
                self.comp_state.dram_state.insert(
                    next_free_row,
                    RowState {
                ...
                program
                    .constants_row_placement
                    .insert(constant, next_free_row);
            }
        }
    }
        ...
        for input in network.leafs().collect::<Vec<Id>>() {
            ...
            if let Some(original_input_locations) = self.signal_to_subarrayids.get(&original_signal)
            {
                for subarray in original_input_locations {
                    let next_free_row = self
                        ...
                        .get_mut(subarray)
                        .and_then(|v| v.pop())
                        .expect("OOM: No more free rows in subarray {subarray} for placing inputs");
                    self.comp_state
                        .value_states
                        .insert((original_signal, *subarray), next_free_row);

                    program
                        .input_row_operands_placement
                        .entry(original_signal)
                        .or_default()
                        .push(next_free_row);
                }
            }
            ...
            if let Some(inverted_input_locations) = self.signal_to_subarrayids.get(&inverted_signal)
            {
                for subarray in inverted_input_locations {
                    ...
                    self.comp_state
                        .value_states
                        .insert((inverted_signal, *subarray), next_free_row);

                    program
                        .input_row_operands_placement
                        .entry(inverted_signal)
                        .or_default()
                        .push(next_free_row);
                }
            }
    ...
    fn init_candidates(&mut self, network: &impl NetworkWithBackwardEdges<Node = Aoig>) {
        ...
        for &input in inputs.as_slice() {
            ...
            let mut outputs_with_prio: PriorityQueue<(Signal, SubarrayId), SchedulingPrio> =
                network
                    .node_outputs(input)
                    .filter(|output| {
                        network
                            ...
                            .inputs()
                            .iter()
                            .all(|other_input| inputs.contains(&other_input.node_id()))
                    }) // only those nodes are candidates, whose src-operands are ALL inputs (->only primary inputs are directly available)
                    .flat_map(|output| {
                        ...
                        for subarray in self
                            .signal_to_subarrayids
                            .get(&output_signal)
                            .expect("Signal is not mapped to a subarray yet??")
                        {
                            output_candidates.push((
                                (output_signal, *subarray),
                        ...
                        if self
                            .signal_to_subarrayids
                            .contains_key(&inverted_output_signal)
                        {
                            // for every subarray in which the signal is needed
                            for subarray in self
                                .signal_to_subarrayids
                                .get(&inverted_output_signal)
                                .expect("Signal is not mapped to a subarray yet??")
                            {
                                output_candidates.push((
                                    (inverted_output_signal, *subarray),
                                    self.compute_scheduling_prio_for_node(
                                        inverted_output_signal,
                                        *subarray,
                                        network,
                                    ),
                                ));
    ...
    fn get_new_candidates(
        ...
        network
            ...
            .filter({
                |out| {
                    network.node(*out).inputs().iter().all(|input| {
                        debug!("OUTPUT: {out:?} with input {input:?}");
                        debug!("Out: {:?}, In: {:?}", out, input);
                        self.comp_state
                            .value_states
                            .contains_key(&(*input, subarray))
                    })
                }
            })
            .flat_map(|id| {
                ...
                let mut new_candidates = vec![(
                    (noninverted_signal, subarray),
                    self.compute_scheduling_prio_for_node(noninverted_signal, subarray, network),
                )]; // noninverted version needs to be computed anyway
                ...
                if self.signal_to_subarrayids.contains_key(&inverted_signal) {
                    new_candidates.push((
                        (inverted_signal, subarray.get_partner_subarray()),
                        self.compute_scheduling_prio_for_node(
                            noninverted_signal,
                            subarray,
                            network,
                ...
                new_candidates
                    .into_iter()
                    .collect::<PriorityQueue<(Signal, SubarrayId), SchedulingPrio>>()
    ...
    fn init_comp_state(
        ...
        let config_file = unsafe { CStr::from_ptr(self.settings.config_file) }
            .to_str()
            .unwrap();
        ...
        if config.is_file() {
            ...
            self.compute_row_activations = HashMap::from([
                (
                    (
                        SupportedNrOperands::One,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(8), RowAddress(8)),
                ),
                (
                    (
                        SupportedNrOperands::One,
                    ...
                    (RowAddress(303), RowAddress(303)),
                ),
                (
                    (
                        SupportedNrOperands::Two,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(15), RowAddress(79)),
                ),
                (
                    (
                        SupportedNrOperands::Two,
                    ...
                    (RowAddress(293), RowAddress(357)),
                ),
                (
                    (
                        SupportedNrOperands::Four,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(60), RowAddress(42)),
                ),
                (
                    (
                        SupportedNrOperands::Four,
                    ...
                    (RowAddress(472), RowAddress(412)),
                ),
                (
                    (
                        SupportedNrOperands::Eight,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(42), RowAddress(15)),
                ),
                (
                    (
                        SupportedNrOperands::Eight,
                    ...
                    (RowAddress(203), RowAddress(283)),
                ),
                (
                    (
                        SupportedNrOperands::Sixteen,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(32), RowAddress(83)),
                ),
                (
                    (
                        SupportedNrOperands::Sixteen,
                    ...
                    (RowAddress(470), RowAddress(252)),
                ),
                (
                    (
                        SupportedNrOperands::Thirtytwo,
                        NeighboringSubarrayRelPosition::Above,
                    ),
                    (RowAddress(307), RowAddress(28)),
                ),
                (
                    (
                        SupportedNrOperands::Thirtytwo,
        ...
        let compute_rows = self.compute_row_activations.values().fold(
            vec![],
            |all_compute_rows, next_compute_row_combi| {
                let new_compute_rows = ARCHITECTURE
                    .precomputed_simultaneous_row_activations
                    .get(next_compute_row_combi)
                    .expect("Compute row can't be activated??");
                all_compute_rows
                    .iter()
                    .chain(new_compute_rows)
                    .cloned()
                    .collect()
            },
        );
        let mut free_rows = (0..ROWS_PER_SUBARRAY)
            .map(RowAddress::from)
            .collect::<Vec<RowAddress>>();
        free_rows.retain(|r| !compute_rows.contains(r));
        for subarray in 0..NR_SUBARRAYS {
            let free_rows_in_subarray = free_rows
                .iter()
                .map(|row| row.local_rowaddress_to_subarray_id(SubarrayId(subarray)))
                .collect(); // transform local row address to row addresses in corresponding `subarray`
            self.comp_state
                .free_rows_per_subarray
                .entry(SubarrayId(subarray))
                .insert_entry(free_rows_in_subarray);
        }
        ...
        debug!(
            "Placed inputs {:?} in {:?}",
            network.leafs().collect::<Vec<Id>>(),
            self.comp_state.value_states
        );
    ...
    fn assign_signals_to_subarrays(
        &mut self,
        network: &impl NetworkWithBackwardEdges<Node = Aoig>,
    ) {
        ...
        for output in network.outputs() {
            self.signal_to_subarrayids
                .insert(output, vec![SubarrayId(subarray_id)]); // determine (virtual) subarray in which output will reside - assignment might change in a later stage by merging several outputs into same subarray if there is enough space
            ...
            let (actual_subarray, neighboring_subarray) = {
                ...
                if output.is_inverted() {
                    self.signal_to_subarrayids.insert(
                        Signal::new(output.node_id(), false),
                        vec![neighboring_subarray],
                    ); // determine (virtual) subarray in which output will reside
            ...
            let (noninverted_src_signals, inverted_src_signals) =
                self.get_all_noninverted_and_inverted_src_signals(output, network);

            println!(
                "Noninverted src signals: {:?}",
                noninverted_src_signals.clone()
            );
            ...
            for connected_signal in noninverted_src_signals {
                self.signal_to_subarrayids
                    .entry(connected_signal)
                    .or_default()
                    .push(actual_subarray); // determine (virtual) subarray in which output will reside
            }
            ...
            let mut unvisited_signals_in_neighboring_subarray: Vec<Signal> = inverted_src_signals
                .iter() // =those signals that are negated an odd nr of times
                ...
                .map(|signal| Signal::new(signal.node_id(), false))
                .collect(); // before negation the signals are in the neighboring subarray
            while !unvisited_signals_in_same_subarray.is_empty()
                || !unvisited_signals_in_neighboring_subarray.is_empty()
            {
                println!("Same subarray: {:?}", unvisited_signals_in_same_subarray);
                println!(
                    "Neighboring subarray: {:?}",
                    unvisited_signals_in_neighboring_subarray
                );
                if let Some(signal_neighboring_subarray) =
                    unvisited_signals_in_neighboring_subarray.pop()
                {
                    debug!("Neighboring: {signal_neighboring_subarray:?}");
                    self.signal_to_subarrayids
                        .entry(signal_neighboring_subarray)
                        .or_default()
                        .push(neighboring_subarray);
                    ...
                    let (signals_neighboring_subarray, signals_inverted_even_nr_times) = self
                        .get_all_noninverted_and_inverted_src_signals(
                            signal_neighboring_subarray,
                            network,
                        );
                    for signal_inverted_odd_nr_times in signals_neighboring_subarray {
                        ...
                        self.signal_to_subarrayids
                            .entry(signal_inverted_odd_nr_times)
                            .or_default()
                            .push(neighboring_subarray);
                    }

                    // signals which are inverted again require the non-inverted version to be in the other subarray
                    let mut signals_to_invert_once_more: Vec<Signal> =
                        signals_inverted_even_nr_times
                            .into_iter()
                            .filter(|signal| {
                ...
                if let Some(signal_same_subarray) = unvisited_signals_in_same_subarray.pop() {
                    ...
                    let (signals_same_subarray_of_output, signals_inverted_even_nr_times) = self
                        .get_all_noninverted_and_inverted_src_signals(
                            signal_same_subarray,
                            network,
                        );
                    ...
                    unvisited_signals_in_neighboring_subarray
                        .append(&mut signals_to_invert_once_more);
                }
            }
    ...
    fn get_all_noninverted_and_inverted_src_signals(
        &mut self,
        signal: Signal,
        network: &impl NetworkWithBackwardEdges<Node = Aoig>,
        ...
        while let Some(src_operand) = stack_unvisited_noninverted_src_operands.pop() {
            if src_operand.is_inverted() {
                ...
                stack_unvisited_noninverted_src_operands
                    .append(&mut Vec::from(src_operand_node.inputs()));
            }
        }

        self.computed_noninverted_scr_signals.insert(
            signal,
            (
                inverted_src_signals.clone(),
                noninverted_src_signals.clone(),
            ),
        ); // to save (possible) recomputation next time
    ...
    fn init_reference_subarray(
        &self,
        mut ref_rows: Vec<RowAddress>,
        logic_op: LogicOp,
    ) -> Vec<Instruction> {
        match logic_op {
            LogicOp::AND => {
                let frac_row = ref_rows
                    .pop()
                    .expect("Min 1 row has to be passed for initializing ref subarray"); // TODO: include success-rate considerations to choose best row to use for storing `V_{DD}/2`
                let row_address_1 = self
                    .comp_state
                    .constant_values
                    .get(&1)
                    .expect("Constants are expected to be placed in every subarray beforehand")
                ...
                for other_row in ref_rows {
                    instructions.push(Instruction::RowCloneFPM(
                        row_address_1,
                        other_row,
                        String::from("Init ref-subarray with 1s"),
                    ));
                }
                instructions
            }
            LogicOp::OR => {
                ...
                for other_row in ref_rows {
                    instructions.push(Instruction::RowCloneFPM(
                        row_address_0,
                        other_row,
                        String::from("Init ref-subarray with 0s"),
                    ));
                }
                instructions
            }
    ...
    fn execute_not(
        &mut self,
        signal_to_invert: &Signal,
        dst_array: SubarrayId,
    ) -> Vec<Instruction> {
        let mut instructions = vec![];
        let row_combi = self
            .compute_row_activations
        ...
        instructions.push(self.execute_intrasubarray_rowclone(
            *src_location,
            src_row,
            String::from("Move into compute row"),
        ));
        ...
        let free_row = self
            ...
            .get_mut(&dst_array)
            .and_then(|v| v.pop())
            .unwrap_or_else(|| panic!("OOM: No free rows in subarray {dst_array}"));
        instructions.push(self.execute_intrasubarray_rowclone(
            dst_row,
            free_row,
            String::from("Move into free row"),
        ));
        self.comp_state
            .value_states
            .insert((*signal_to_invert, dst_array), free_row);
    ...
    fn execute_and_or(
        &mut self,
        node_id: Id,
        compute_subarray: SubarrayId,
        network: &impl NetworkWithBackwardEdges<Node = Aoig>,
    ) -> Vec<Instruction> {
        let mut instructions = vec![];

        let (reference_subarray, rel_position_sense_amps_to_compute_subarray) =
            if compute_subarray.0 % 2 == 0 {
                // currently the following arrays are compute&reference subarrays of each other: 0&1,2&3,4&5,...
                (
                    compute_subarray.0 + 1,
                    NeighboringSubarrayRelPosition::Below,
                )
            } else {
                (
        ...
        let nr_src_operands =
            SupportedNrOperands::try_from(network.node(node_id).inputs().len()).unwrap();
        let compute_row_combi = self
            .compute_row_activations
            .get(&(nr_src_operands, rel_position_sense_amps_to_compute_subarray))
            .expect("Nr input operands is {}");
        let compute_rows = ARCHITECTURE
            .precomputed_simultaneous_row_activations
            .get(compute_row_combi)
            .unwrap();
        ...
        let ref_rows = compute_rows
            .iter()
            .map(|c| c.local_rowaddress_to_subarray_id(SubarrayId(reference_subarray)))
            .collect();
        instructions.append(&mut self.init_reference_subarray(ref_rows, logic_op));

        // 2. Execute actual computation
        instructions.push(Instruction::ApaAndOr(
            compute_row_combi
                .0
                .local_rowaddress_to_subarray_id(compute_subarray),
            compute_row_combi
                .1
                .local_rowaddress_to_subarray_id(SubarrayId(reference_subarray)),
        ));
        ...
        let free_row = self
            .comp_state
            .free_rows_per_subarray
            .get_mut(&compute_subarray)
            .and_then(|v| v.pop())
            .unwrap_or_else(|| panic!("No more free rows in subarray {compute_subarray}"));
        instructions.push(self.execute_intrasubarray_rowclone(
            compute_rows[0],
            free_row,
            String::from("Move into free row"),
        ...
        self.comp_state
            .value_states
            .insert((Signal::new(node_id, false), compute_subarray), free_row); // TODO: for inverted signals not in result_subarray, right?

        instructions
    }

    fn execute_intrasubarray_rowclone(
        &self,
        src_row: RowAddress,
        dst_row: RowAddress,
        comment: String,
    ...
    fn execute_next_instruction(
        &mut self,
        next_candidate: &(Signal, SubarrayId),
        network: &impl NetworkWithBackwardEdges<Node = Aoig>,
    ) -> Vec<Instruction> {
        ...
        let compute_subarray = if signal.is_inverted() {
            result_subarray.get_partner_subarray()
        } else {
            *result_subarray
        }; // for inverted signals first compute the noninverted signal in the other subarray
    ...
    fn compute_scheduling_prio_for_node(
        ...
        let nr_last_value_uses = network
            .node(signal.node_id())
            .inputs() // for each input check whether `id` is the last node using it
            .iter()
            .fold(0, |acc, input| {
                let input_id = Signal::node_id(input);
                let non_computed_outputs: Vec<Id> = network
                    .node_outputs(input_id) // get all other nodes still relying on this input
                    .filter(|out| {
                        let out_signal = Signal::new(*out, false);
                        out_signal != signal && // all output signals except for the current one
                        !(self.comp_state.value_states.contains_key(&(out_signal, subarray)))
                        // that are not yet computed (not rows present in `subarray` holding that value
...
impl Ord for SchedulingPrio {
    fn cmp(&self, other: &Self) -> Ordering {
        self.nr_last_value_uses
            .cmp(&other.nr_last_value_uses)
...
mod tests {
    ...
    fn init() -> Compiler {
        ...
        Compiler::new(CompilerSettings {
            print_program: true,
            verbose: true,
            print_compilation_stats: false,
            min_success_rate: 0.999,
            repetition_fracops: 5,
            safe_space_rows_per_subarray: 16,
            config_file: CString::new(
                "/home/alex/Documents/Studium/Sem6/inf_pm_fpa/lime-fork/config/fcdram_hksynx.toml",
            )
            .unwrap()
            .as_ptr(),
    ...
    fn test_candidate_initialization() {
        ...
        let my_expression: RecExpr<AoigLanguage> = "(and (and 1 3) (and 2 3))".parse().unwrap();
        egraph.add_expr(&my_expression);
        let output2 = egraph.add(AoigLanguage::And([
            eggmock::egg::Id::from(0),
            eggmock::egg::Id::from(2),
        ])); // additional `And` with one src-operand=input and one non-input src operand
