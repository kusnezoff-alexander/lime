...
use eggmock::{egg::Language, AoigLanguage};
use log::debug;
...
use super::architecture::{
    FCDRAMArchitecture, LogicOp, NeighboringSubarrayRelPosition, RowAddress,
    RowDistanceToSenseAmps, SuccessRate,
};
...
impl ops::Add<InstructionCost> for InstructionCost {
    ...
    fn add(self, rhs: InstructionCost) -> Self::Output {
        if self.success_rate.0.abs() > 1.0 || rhs.success_rate.0.abs() > 1.0 {
            // program_cost > 0 since `usize` is always non-negative
...
impl Ord for InstructionCost {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        if self.success_rate == other.success_rate {
            // TOOD: cmp based on some margin (eg +-0.2%)
...
impl CostFunction<AoigLanguage> for CompilingCostFunction {
        ...
        C: FnMut(Id) -> Self::Cost,
    {
        ...
        // see Figure17 [1]
        let and_nr_operand_to_success_rate: HashMap<usize, f64> =
            HashMap::from([(2, 0.85), (4, 0.88), (8, 0.9), (16, 0.92)]); // read from graph (only estimates anyway)
        let or_nr_operand_to_success_rate: HashMap<usize, f64> =
            HashMap::from([(2, 0.88), (4, 0.9), (8, 0.96), (16, 0.98)]); // read from graph (only estimates anyway)
        ...
        let op_cost = match *enode {
            ...
            | AoigLanguage::And4(_)
            | AoigLanguage::And8(_)
            | AoigLanguage::And16(_)
            | AoigLanguage::And32(_) => {
                let mem_cycles_and =
                    FCDRAMArchitecture::get_instructions_implementation_of_logic_ops(LogicOp::AND)
                        .iter()
                        .fold(0, |acc, instr| acc + instr.get_nr_memcycles() as usize);
                ...
                let &success_rate_operand =
                    and_nr_operand_to_success_rate.get(&nr_operands).unwrap();
                InstructionCost {
            ...
            AoigLanguage::Not(_) => {
                let mem_cycles_not =
                    FCDRAMArchitecture::get_instructions_implementation_of_logic_ops(LogicOp::NOT)
                        .iter()
                        .fold(0, |acc, instr| acc + instr.get_nr_memcycles() as usize);
